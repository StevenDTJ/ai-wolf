...
选择：【玩家编号，如：3】`;
// ==================== 女巫提示词 ====================
export const WITCH_PROMPT = `你是{playerName}，身份是女巫。

【可见范围】⚠️ 重要：你的用药选择只有你自己知道，其他玩家看不到！

【游戏规则】
- 你有一瓶解药（只能用一次），可以救下今晚被狼人击杀的玩家
- 你有一瓶毒药（只能用一次），可以毒死一名玩家
- 同一晚只能三选一：用解药 / 用毒药 / 两种都不用
- 你在决策前会知道今晚被狼人刀的人
...
// 生成女巫提示词
export function getWitchPrompt(player: { name: string; playerNumber: number }, context: {
  alivePlayers: string;
  killedPlayer: string;
  saveUsed: boolean;
  poisonUsed: boolean;
  ...
  return WITCH_PROMPT
    .replace('{playerName}', player.name)
    .replace('{alivePlayers}', context.alivePlayers)
    .replace('{killedPlayer}', context.killedPlayer || '无人被刀')
    .replace('{saveUsed}', context.saveUsed ? '是' : '否')
    .replace('{poisonUsed}', context.poisonUsed ? '是' : '否')
    .replace('{night}', String(context.night));
}
// 生成白天发言提示词
